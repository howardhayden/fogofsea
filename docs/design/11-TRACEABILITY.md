...
| Upstream-authorized contact estimates require sensing | Current play reports no estimate and renders zero markers; pure fixtures bound explicitly supplied shapes without proving authority | `contactVisualization.ts`, `Battlefield.tsx` | Contact-visualization unit tests and zero-state browser integration | Capability invents a position, adapter validation is called provenance, identity/composition leaks, or an unsupported supplied marker appears |
