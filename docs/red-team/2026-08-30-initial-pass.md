...
| RT-INFO-009 | Positive control | The projection sanitizer does not encode exact hidden enemy composition, but the live application currently supplies no coordinate-bearing estimate records. | Retain the zero-state and sanitizer. Require a separately versioned, player-visible authority source before any positive marker integration. | Pure projection tests and zero-state browser coverage exist; positive end-to-end behavior remains unimplemented. |
