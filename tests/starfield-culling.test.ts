...
test("STAR-PERF-01: all 15,360 canonical lights remain; only provably outside envelopes are not submitted", () => {
  ...
  for (let i = 0; i < plan.stars.length; i++) {
    sourceByMatrix.set(originals[0].slice(i * 16, i * 16 + 16).join(","), i);
  }
  const compactedAttributes = attributes(mesh);
  compactedAttributes.forEach((attribute) => {
    assert.deepEqual(attribute.updateRanges, [{ start: 0, count: mesh.count * attribute.itemSize }]);
  });
  let previousSource = -1;
  for (let compacted = 0; compacted < mesh.count; compacted++) {
    const source = sourceByMatrix.get(Array.from(mesh.instanceMatrix.array.slice(compacted * 16, compacted * 16 + 16)).join(","));
    assert.notEqual(source, undefined, `unknown compacted matrix ${compacted}`);
    assert.ok(source! > previousSource, "compacted instances preserve canonical order");
    previousSource = source!;
    compactedAttributes.forEach((attribute, attributeIndex) => {
      const stride = attribute.itemSize;
      assert.deepEqual(
        Array.from(attribute.array.slice(compacted * stride, compacted * stride + stride)),
        originals[attributeIndex].slice(source! * stride, source! * stride + stride),
        `attribute ${attributeIndex} remains paired at compacted index ${compacted}`,
      );
    });
  }
...
test("STAR-PERF-02: camera turns restore canonical data in order and stationary animation uploads nothing", () => {
  ...
  camera.position.set(13, -7, 29);
  updateStarfield(runtime, 1, false, camera.position);
  prepareStarfieldForCamera(runtime, camera);
  assert.deepEqual(attributes(mesh).map(a => a.version), versions, "camera-follow translation preserves the same local frustum");
  ...
  prepareStarfieldForCamera(runtime, camera);
  assert.equal(mesh.count, 0, "a valid empty frustum submits no instances");
  camera.far = 450;
  camera.updateProjectionMatrix();
  prepareStarfieldForCamera(runtime, camera);
  assert.equal(mesh.count, count);
...
test("STAR-PERF-03: conservative envelopes retain sampled full-facet pulse/wander extremes across seams, FOV and roll", () => {
  const { runtime, camera, mesh } = fixture();
  ...
  for (let pose = 0; pose < 8; pose++) {
    ...
    for (let i = 0; i < runtime.plan.stars.length; i += 7) {
      ...
      for (let sign = 0; sign < 8; sign++) for (let v = 0; v < vertices.count; v++) {
