...
test("selected operational subjects breathe faintly while stars remain a separate motion channel", async ({ page }) => {
  ...
  await expect(plot).toHaveAttribute("data-dream-emission-halo", "compact-source-convolution");
  await expect(plot).toHaveAttribute("data-dream-emission-occlusion", "visible-source-and-destination-depth");
  await expect(page.locator("#weather-visual-note")).toContainText("short silhouette-derived glow");
  await expect(page.locator("#weather-visual-note")).toContainText("depth and fog still restrict it");
  ...
  await expect(plot).toHaveAttribute("data-dream-emission-fallback", "core-only");
  await expect(plot.locator(".fallback-dream-halo")).toHaveCount(0);
  await expect(plot).toHaveAttribute("data-ndcg-render-profile", "ndcg-v0.1-native-candidate");
  expect(Number(await plot.getAttribute("data-ndcg-registered-subjects"))).toBeGreaterThanOrEqual(unitCount);
