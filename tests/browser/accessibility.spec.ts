...
test("academy uses keyboard-operated tabs, named panels, quiz groups, and modal focus containment", async ({ page }) => {
  ...
  await expect(academy.locator("#academy-summary")).toHaveCount(1);
  ...
  await expect.poll(visiblePanels).toEqual(["academy-panel-guide"]);
  ...
  const now = academy.getByRole("tab", { name: "NOW", exact: true });
  const library = academy.getByRole("tab", { name: "LIBRARY", exact: true });
  const compare = academy.getByRole("tab", { name: "COMPARE", exact: true });
  const sources = academy.getByRole("tab", { name: "SOURCES", exact: true });
  await now.focus();
  await now.press("ArrowRight");
  await expect(library).toBeFocused();
  await expect(library).toHaveAttribute("aria-selected", "true");
  await expect(academy.getByRole("tabpanel", { name: "LIBRARY" })).toBeVisible();
  ...
  await expect(academy.getByRole("tabpanel", { name: "SOURCES" })).toBeVisible();
  await expect.poll(visiblePanels).toEqual(["academy-panel-sources"]);
  await sources.press("Home");
  await expect(now).toBeFocused();
  await expect(academy.getByRole("tabpanel", { name: "NOW" })).toBeVisible();
  await expect.poll(visiblePanels).toEqual(["academy-panel-guide"]);
  await now.press("ArrowRight");
  await expect(library).toBeFocused();
  await expect(academy.getByRole("tabpanel", { name: "LIBRARY" })).toBeVisible();
  await expect.poll(visiblePanels).toEqual(["academy-panel-course"]);
  const lessonBody = academy.locator(".lesson-body");
  const lessonSummary = lessonBody.locator(":scope > summary");
  await lessonSummary.focus();
  await lessonSummary.press("Enter");
  await expect(lessonBody).toHaveAttribute("open", "");
  const knowledgeCheck = academy.locator(".knowledge-check");
  const knowledgeCheckSummary = knowledgeCheck.locator(":scope > summary");
  await knowledgeCheckSummary.focus();
  await knowledgeCheckSummary.press("Enter");
  await expect(knowledgeCheck).toHaveAttribute("open", "");
