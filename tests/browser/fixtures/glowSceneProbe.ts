...
export function probeGlowScene(time: DreamEmissionTime, alpha: boolean, pixelRatio = 1.8) {
  ...
  return { time, alpha, pixelRatio, available: true, status, actualContextAlpha: context.getContextAttributes()?.alpha, beforeCore, afterCore, beforeBackground, afterBackground, darkenedPixels, litPixels, totalPixels: size.x * size.y, failures, readError, beforeCapture, afterCapture };
}
