...
- No finite scenario list: compositional fictional mission-family grammars—including coastal safeguarding and victim-centered anti-trafficking operations—regenerate the operation, strategic frame, climate, region, season and date, time, weather, clouds, precipitation, sea, wind, current, visibility, celestial eligibility, geography, threat, actors, force requirements, objective, difficulty matrix, civilian context, intelligence, and constraints for every new game. A pure whole-candidate validator accepts the scenario only when its implemented structured coexistence checks pass; rejected candidates are discarded and regenerated from fresh entropy, with a bounded deterministic constructive fallback that must pass the same checks before presentation. Those checks do not establish that every accepted scenario is winnable, nontrivial, doctrinally sound, or politically coherent, and they do not validate every implication of free-form narrative
...
- At Dawn, Dusk, and Night, authorized visible vessels, submarines, aircraft, and creatures seed a bounded, view-conditioned three-scale glow through renderer-owned emission-only proxies that sample their existing native colors and textures. Production geometry and directly lit materials remain unchanged; scene depth and fog attenuate the exterior field, side-on foreshortening contracts its spread, 31- and 47-second phases modulate gain only, and reduced motion holds that gain steady. Day is off, capability or budget fallbacks retain the direct core-only scene, no poor-weather lift is applied, and the effect creates no halo meshes or point lights
...
- Progressive disclosure keeps current conditions, current decisions, and the next action visible while mission detail, reference material, reports, scoring breakdowns, and nonsuggested Academy reading begin collapsed behind keyboard-native summaries; a bounded scenario-relevant lesson set may begin open
...
- An evidence-led command intelligence surface: Turn 1 establishes the baseline; Turns 2–6 require bounded, non-scoring staff judgments about possible opposing intent, the observable pattern, and a likely next action. A matching left desktop panel separates absolutely known observations from potentials, keeps the latest known opposing action and modeled inflictions in Immediate, and files late discoveries under their occurrence turn as **Discovered during Turn N**. Compact layouts preserve the same intelligence-before-orders sequence in one vertical scroll path
- A bounded compound-uncertainty model with precommitted internal draws, severe-weather and institutional disruptions, multiple or emergent objectives, opposing cooperation, and independent opportunist actors. Ordinary turns never preview adjudication: after resolution they give one concise learning note that distinguishes a tracked task, reach, contact, or employment mismatch from a committed matrix failure or the absence of a tracked turn mismatch. The Field Guide explains the model in plain language without treating an absent finding as proof of correct play or presenting internal draws as measured real-world odds
...
- The plot's unknown-contact boundary shape-checks caller-supplied disclosed-estimate records and then filters them through mission-credited, compatible detection capability; upstream code remains responsible for establishing their authority. The current scenario model supplies no coordinate-bearing estimates, so live play correctly renders zero markers even with credited sensing and capability never invents a location. Pure fixtures bound any future supplied marker to its domain and keep identity and opposing composition absent
...
- Human-readable `.txt` export containing difficulty, the accepted environment, decisions, force design, every player-visible command turn, recorded staff judgments, the as-known intelligence log, written logic, assumptions, and termination criteria. Concealed actions, future commitments, internal matrix values, and raw umpire notes remain outside the readable record; the complete restorable machine block is encoded rather than encrypted whenever a command matrix exists
...
The academy is an original independent synthesis that provides declared primary and secondary reading trails. It does not provide academic credit or certification. The current automated curriculum-data lint does not authenticate complete source coverage, citation quality, lesson accuracy, quiz validity, or learner comprehension.
...
The owner-package digest is SHA-256 over a closed stable-JSON manifest of the
...
copy where a register treatment could weaken immediate meaning.
...
`RELEASE_QA.md` and [`docs/red-team/2026-08-30-initial-pass.md`](docs/red-team/2026-08-30-initial-pass.md) retain earlier acceptance and adversarial evidence. [`docs/red-team/2026-09-01-lattice-copy-pass.md`](docs/red-team/2026-09-01-lattice-copy-pass.md) records the bounded Lattice gate, correction evidence, and unresolved expansion blockers. `ACCESSIBILITY.md` documents keyboard behavior, screen-reader structure, visual accommodations, and the explicit limits of the current automated evidence. `SECURITY.md` documents the input trust boundary, local storage, response policy, and remaining limits. `PLAYTEST_PROTOCOL.md` provides the human comprehension instrument for testing whether players understand force fit instead of chasing a score; it does not fabricate participant findings.

The [product, UX, and design documentation suite](docs/design/README.md) specifies the as-built design-system architecture, visual and motion language, information architecture, complete interaction model, persuasive and emotional design, persona journeys and empathy maps, local-first service blueprint, UX roadmap, security/privacy/TXT lifecycle, graphics and decision logic, Gestalt analysis, informative microcopy, heuristic evaluation, requirement traceability, the bounded Lattice copy architecture, and the typed turn-intelligence contract. Roadmap material and research hypotheses are explicitly separated from implemented or observed behavior.
